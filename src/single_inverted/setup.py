...
setup(
    ...
    description='Single inverted pendulum: sim, PID/MPC controllers, evaluator.',
    license='MIT',
    tests_require=['pytest'],
    # evaluate.py is installed as a raw executable script (ros2 run
    # single_inverted evaluate.py ...), NOT a console_scripts entry point,
    # so its location stays exactly at scripts/evaluate.py per the repo
    # layout in README.md Sec 2.
    scripts=['scripts/evaluate.py'],
    entry_points={
        'console_scripts': [
            'sim_node = single_inverted.sim_node:main',
            'controller_pid = single_inverted.controller_pid:main',
            'controller_mpc = single_inverted.controller_mpc:main',
        ],
    },
)
